...
// Update user details
if (isset($_GET['q']) && $_GET['q'] == '4' && isset($_POST['update'])) {
    // Sanitize form inputs
    $new_name = sanitizeInput($_POST['name']);
    $new_password = sanitizeInput($_POST['password']);
    $new_mob = sanitizeInput($_POST['mob']);  // mobile number if you have it
    $current_email = $_SESSION['email'];  // use session email for security
    
    // Using prepared statement for better security
    $stmt = $con->prepare("UPDATE user SET name = ?, password = ?, mob = ? WHERE email = ?");
    $stmt->bind_param("ssss", $new_name, $new_password, $new_mob, $current_email);
    
    if ($stmt->execute()) {
        header("location:account.php?q=4&msg=Profile Updated Successfully");
        exit();
    } else {
...
// Delete feedback
if (isAdmin() && isset($_GET['fdid'])) {
    $id = sanitizeInput($_GET['fdid']);
    mysqli_query($con, "DELETE FROM feedback WHERE id='$id'") or die('Error deleting feedback');
