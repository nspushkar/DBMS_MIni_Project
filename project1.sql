...
-- Define Triggers
DELIMITER //
CREATE TRIGGER AfterUserInsert AFTER INSERT ON `user`
FOR EACH ROW
BEGIN
    INSERT INTO `history` (`email`, `eid`, `score`, `level`, `sahi`, `wrong`, `date`)
    VALUES (NEW.email, 'Insert', 0, 0, 0, 0, NOW());
END; //
DELIMITER ;  
...
DELIMITER //
CREATE PROCEDURE `AddNewUser` (IN `p_name` VARCHAR(50), IN `p_email` VARCHAR(50), IN `p_password` VARCHAR(50))
BEGIN
...
SELECT `name`, `email`, `college` FROM `user`;
